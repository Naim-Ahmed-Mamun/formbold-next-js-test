...
const Code = ({ code }) => {
  ...
  return (
    <pre
      className={`relative language-${code?.language || "html"}`}
      data-language={code?.language || ""}
    >
      <code className={`language-${code?.language || "html"}`}>
        {code?.code}
      </code>
      <button
        type="button"
        onClick={handleCopy}
        className={`absolute right-2 top-2 z-10 flex h-9 items-center justify-center rounded-lg border border-solid bg-stroke px-5 text-sm text-body-color transition hover:border-primary hover:bg-primary hover:text-white dark:bg-white dark:hover:bg-primary ${
