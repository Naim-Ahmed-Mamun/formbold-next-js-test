  ...
  <div className="items-center justify-center pb-12 sm:flex">
    {billing ? (
      ...
      >
        ...
        >
          Yearly{" "}
          <span className={`pl-1 ${yearly ? "text-white" : "text-primary"}`}>
            {" "}
            (Save Over 30%){" "}
          </span>
        </button>
      ...
      <div
        className={`inline-flex h-[58px] items-center justify-between rounded-[60px] bg-fb-gray p-1.5`}
      >
        <button
          className={`relative flex h-11 items-center rounded-3xl px-6 font-heading text-base font-medium ${
            yearly
              ? "cursor-pointer text-black"
              : "bg-white text-primary shadow-fb-three"
          }`}
          onClick={() => setYearly(false)}
        >
          Monthly
    ...
    {billing && !isNull(updatePaymentInfoUrl) && (
      <a
        href={updatePaymentInfoUrl}
        target="_blank"
        className="mt-5 flex items-center rounded-lg bg-primary px-4 py-2 font-medium text-white sm:ml-4 sm:mt-0"
      >
        Update Payment Info
        <svg
          xmlns="http://www.w3.org/2000/svg"
...
export default function PricingPlan(props) {
  ...
  const finalPlans = useMemo(
    () =>
      plans.filter((plan) => plan.price != 0 && !plan.is_monthly === yearly),
    [plans, yearly]
  );
    ...
    <>
      <div className="mx-auto mb-[60px] w-full max-w-[1310px] px-4">
        <PricingPlanHeader
          ...
          yearly={yearly}
          setYearly={setYearly}
          updatePaymentInfoUrl={hasSubscription ? updatePaymentInfoUrl : null}
        />

        <div
          className={`rounded-[22px] border border-fb-gray-2 md:mt-14 ${
            props?.billing ? "bg-white" : "bg-fb-gray"
          }`}
        >
          <div
            className={`${
            ...
            {pricingPage?.pageLoader ? (
              ...
              finalPlans.map((plan, index) => (
                <SinglePricingPlan
                  ...
                  loading={
                    loading.plan === plan.paddle_plan ? loading.loader : false
                  }
                  yearly={yearly}
                />
              ))
