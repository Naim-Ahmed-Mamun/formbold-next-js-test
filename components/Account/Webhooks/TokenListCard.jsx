...
const TokenListCard = () => {
  ...
  const handelTokenDelete = (e, token) => {
    e.preventDefault();
    dispatch(revokeUserApiToken(token?.id));
  };
  ...
  return (
    <div className="rounded-[10px] bg-white p-6 shadow-fb-one sm:p-10 xl:p-8 2xl:p-10">
      ...
      {webhooks.tokens && !isEmpty(webhooks.tokens) ? (
        <table className="w-full table-auto">
          <thead>
            <tr className="rounded-t-[10px] border-b border-fb-stroke">
              <th className="min-w-[130px] px-6 py-5 text-left font-heading text-sm font-medium text-body-color">Name</th>
              <th className="min-w-[140px] px-6 py-5 text-left font-heading text-sm font-medium text-body-color">Date</th>
              <th className="min-w-[140px] px-6 py-5 text-left font-heading text-sm font-medium text-body-color">Token</th>
              <th className="min-w-[180px] px-6 py-5 text-left font-heading text-sm font-medium text-body-color">Action</th>
            </tr>
          </thead>
          <tbody>
            {webhooks.tokens.map((token, index) => (
              <tr key={index} className="border-b border-stroke last-of-type:border-b-0">
                ...
                <td className="p-5 text-sm text-black relative">
                  {/* Masked Token */}
                  <div className="token-mask">
                    {token?.token && (
                      <span className="masked-text" title={token.token}>
                        ************
                      </span>
                    )}
                  </div>
                ...
                <td className="p-5 text-left text-base">
                  <button
                    onClick={(e) => handelTokenDelete(e, token)}
                    className="font-sm inline-flex h-[34px] items-center rounded-[5px] bg-red-400 px-4 font-heading font-medium text-white duration-300 hover:bg-red-500"
                    target="_blank"
                    rel="noreferrer"
                  >
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      />
                      <path
                        d="M7.85488 8.5448C8.19834 8.51045 8.50462 8.76104 8.53897 9.10451L8.95563 13.2712C8.98998 13.6146 8.73939 13.9209 8.39592 13.9553C8.05246 13.9896 7.74618 13.739 7.71184 13.3956L7.29517 9.22889C7.26082 8.88542 7.51141 8.57915 7.85488 8.5448Z"
                        fill="currentColor"
                      />
                      <path
        ...
        <p className="mb-7.5 text-sm text-body-color">Nothing yet 👀</p>
      )}
    </div>
  );
};
