import { getAllPosts, getPostBySlug, markdownToHtml } from '../../../../data/helper';
import { siteURL } from '../../../../services/config';
import AlternativeDetailsMain from './_components/AlternativeDetailsMain';
...
export async function generateMetadata({ params }) {
  ...
  // Derive asPath equivalent (e.g., /alternatives/some-post)
  const asPath = `/alternatives/${slug}`;
...
export async function generateStaticParams() {
  const posts = getAllPosts(["slug"], "alternatives");
...
export default async function AlternativeDetailsPage({ params }) {
  const { slug } = await params;
  const data = await getPostBySlug(
    slug,
    ["title", "description","heading","subheading", "icon", "author", "content", "ogImage"],
    "alternatives"
  );
  ...
  return <AlternativeDetailsMain post={post} />;
}
